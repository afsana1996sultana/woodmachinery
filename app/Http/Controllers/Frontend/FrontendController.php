...
class FrontendController extends Controller
{
    ...
    public function VendorWiseProduct(Request $request,$slug){
        ...
        $products = Product::where('status', 1)->where('approved',1)->where('vendor_id',$vendor->id)->orderBy('id','DESC')->paginate(20);
        // Price Filter
        if ($request->get('filter_price_start')!== Null && $request->get('filter_price_end')!== Null ){ 
            ...
            if ($filter_price_start>0 && $filter_price_end>0) {
                $products = Product::where('status','=',1)->where('approved',1)->where('vendor_id',$vendor->id)->whereBetween('regular_price',[$filter_price_start,$filter_price_end])->paginate(20);
                // dd($products);
            }
