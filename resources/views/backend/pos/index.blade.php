                            ...
                            <div class="col-sm-2 col-xs-6 product-thumb product-row-list" onclick="addToListVariant({{ $product->id }},{{ $stock->id }})">
                                <div class="card mb-4">
                                    <div class="card-body position-relative">
                                        ...
                                        <p style="font-size: 10px; font-weight: bold; line-height: 10px; padding-top: 30px;">
                                            Size: {{ $stock->varient }}
                                        </p>
                                        <p style="font-size: 10px; font-weight: bold; line-height: 10px; padding-top: 30px;">
                                            Stock: {{ $stock->qty }}
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('pos.store') }}" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                <input type="submit" target="_blank" class="btn btn-primary" value="Place Order" style="float: right;">
                            </div>
                        </div>
                    </form>
                </div>
...
@push('footer-script')
    ...
    <script>
        ...
        function addToList(id){
            ...
            $.ajax({
                ...
                success:function(data){
                    //console.log(data);
                    var qty = 0;
                    var stock_qty = data.stock_qty;
                    var aa = stock_qty-1;
                    
                    //console.log(aa);
                    // for (let i = 0; i <= data.stock_qty; i++) {
                    //     console.log(i);
                    // }
                    if(data.stock_qty <= 0){
                        const Toast = Swal.mixin({
                    ...
                    if(data.stock_qty<=0){
                        ...
                        html +=                        `<input type="hidden" name="product_id[]" value="${data.id}">
                                            ...
                                            <div class="col-3">
                                                ...
                                                <input type="hidden" name="pv_id[]" id="pv_id_${vid}" value="${data.varient}">
                                            </div>
                                            <div class="col-2">
        ...
        function filterByBarcode() {
            ...
            $.ajax({
                type:'GET',
                url:'/admin/pos/barcode-product-search/'+search_term_barcode,
                dataType:'json',
                success:function(data){
                    //console.log(data); 
                    var html = '';
                    ...
                    if(Object.keys(data).length > 0){
                        $.each(data, function(key,value){
                            ...
                            if(value.is_varient == 0){
                                ...
                                html  +=                    `<img class="default-img" src="/${value.image}" alt="" />`;
                                                        }else{
                                html  +=                     `<img class="default-img" src="/upload/no_image.jpg" alt="" />`;
                                                        }
                                html  +=            `</div>
        ...
        function barcode() {
            ...
            $.ajax({
                ...
                success:function(abc){
                    //console.log(abc); 
                    if(abc.varient){
                        addToListVariant(abc.product_id,abc.id);
                    }else{
                        addToList(abc.id);
                    }
