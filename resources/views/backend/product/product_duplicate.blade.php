...
<section class="content-main">
    <div class="content-header">
        <h2 class="content-title">Duplicate Product</h2>
        <div class="">
									...
									<tbody>
										@foreach($product->stocks as $stock)
											<tr>
												...
												<td><input type="text" name="{{ $stock->id }}_purchesprice" class="form-control vdpurchase_price" value="{{ $stock->purchesprice }}" required></td>
												<td><input type="text" name="{{ $stock->id }}_price" class="form-control vdp" value="{{ $stock->price }}" required></td>
												
												<td><input type="text" name="{{ $stock->id }}_bodyrate" class="form-control vdp" value="{{ $stock->bodyrate }}" required></td>
												<td><input type="text" name="{{ $stock->id }}_finishingrate" class="form-control vdp" value="{{ $stock->finishingrate }}" required></td>
												
												
												<td><input type="text" name="{{ $stock->id }}_wholesaleprice" class="form-control vdwholesaleprice" value="{{ $stock->wholesaleprice }}" required></td>
												<td><input type="text" name="{{ $stock->id }}_wholesaleqty" class="form-control vdwholesaleqty" value="{{ $stock->wholesaleqty }}" required></td>
												...
												<td><input type="text" name="{{ $stock->id }}_dis_price" class="form-control" value="{{ $stock->dis_price }}" required></td>
												<td>
												...
												<td>
													<img src="{{ asset($stock->manufimage) }}" alt="{{ $stock->varient }}-vmanufimage" style="width: 15%; float: left;">
													<input type="file" name="{{ $stock->id }}_vmanufimage" class="form-control" style="width: 80%; float: left; margin-left: 5%;">
												</td>
											</tr>
										@endforeach
									</tbody>
                        ...
                        <div class="mb-4">
                        	<div class="row">
                          		<div class="custom-control custom-switch">
                                    <input type="checkbox" class="form-check-input me-2 cursor" name="is_dealer" id="is_dealer" {{ $product->is_dealer == 1 ? 'checked': '' }} value="1">
				...
				if(Object.keys(response.data).length > 0) {
					...
					$('#combination_table tbody').html($.map(response.data, function (item, index) {
						return `<tr>
									...
									<td><input type="text" name="vpurchesprices[]" class="form-control vdpurchase_price" value="`+purchase_price+`" required></td>
									...
									<td><input type="text" name="vwhole_sell_prices[]" class="form-control vdwhole_sell_price" value="`+whole_sell_price+`" required></td>
									<td><input type="text" name="vwhole_sell_qtys[]" class="form-control vdwhole_sell_qty" value="`+whole_sell_qty+`" required></td>
...
<script type="text/javascript">
	...
	$('#purchase_price').on('keyup', function() {
    	...
    	$('.vdpurchase_price').val(price);
    });
	$('#whole_sell_qty').on('keyup', function() {
    	var qty = $('#whole_sell_qty').val();
    	$('.vdwhole_sell_qty').val(qty);
    });
	$('#whole_sell_price').on('keyup', function() {
    	var price = $('#whole_sell_price').val();
    	$('.vdwhole_sell_price').val(price);
    });
</script>
