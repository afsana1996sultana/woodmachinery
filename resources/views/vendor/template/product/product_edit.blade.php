...
<section class="content-main">
    <div class="content-header">
        ...
        <div class="">
            <a href="{{ route('vendor.product.all') }}" class="btn btn-primary"><i class="material-icons md-plus"></i> Product List</a>
        </div>
    </div> 
	<div class="row">
		{{-- @if ($errors->any())
								...
								<div class="custom_select">
									<select class="form-control select-active w-100 form-select select-nice" name="category_id" id="product_category">
										@foreach ($categories as $category)
											...
											@foreach ($category->childrenCategories as $childCategory)
												@include('vendor.template.include.child_category', ['child_category' => $childCategory])
											@endforeach
										@endforeach
									</select>
								</div>
...
<script type="text/javascript">
	$(document).ready(function (e) {
			...
			$.ajax({
				type:'POST',
				url: "{{ route('vendor.category.ajax.store') }}",
				data: formData,
				cache:false,
				contentType: false,
				processData: false,
...
<script type="text/javascript">
	$(document).ready(function (e) {
			...
			$.ajax({
				type:'POST',
				url: "{{ route('vendor.brand.ajax.store') }}",
				data: formData,
				cache:false,
				contentType: false,
				processData: false,
...
<script type="text/javascript">
    function productRemove(id){
        ...
        $.ajax({
           type:'GET',
           url:"/vendor/product/multiimg/delete/"+id,
           dataType: 'json',
           success:function(data){
             location.reload();
           	//console.log(data);
