...
<section class="content-main">
    <div class="content-header">
        ...
        <div>
            <a href="{{ route('vendor.product.add') }}" class="btn btn-primary"><i class="material-icons md-plus"></i>Add Product</a>
        </div>
    </div>
    </div>
    <div class="card mb-4">
                        ...
                        <tr>
                            ...
                            <td>
                                @if($item->is_featured == 1)
                                  <a href="{{ route('vendor.product.featured',['id'=>$item->id]) }}">
                                    <span class="badge rounded-pill alert-success"><i class="material-icons md-check"></i></span>
                                  </a>
                                @else
                                  <a href="{{ route('vendor.product.featured',['id'=>$item->id]) }}" > <span class="badge rounded-pill alert-danger"><i class="material-icons md-close"></i></span></a>
                                @endif
                            </td>
                            <td>
                                @if($item->status == 1)
                                  <a href="{{ route('vendor.product.in_active',['id'=>$item->id]) }}">
                                    <span class="badge rounded-pill alert-success">Active</span>
                                  </a>
                                @else
                                  <a href="{{ route('vendor.product.active',['id'=>$item->id]) }}" > <span class="badge rounded-pill alert-danger">Disable</span></a>
                                @endif
                            </td>
                            <td>
                                @if($item->approved != 0)
                            ...
                            <td class="text-end">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href="{{ route('vendor.product.edit',$item->id) }}">Edit info</a>
                                        <!-- <a class="dropdown-item" href="">View Details</a> -->
                                        <a class="dropdown-item text-danger" href="{{ route('vendor.product.delete',$item->id) }}" id="delete">Delete</a>
                                    </div>
                                </div>
                                <!-- dropdown //end -->
                            </td>
